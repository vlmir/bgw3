...
func saveOneUniprot(txid string, datdir string) error {
	// works for only 250 entries
	uri := "https://rest.uniprot.org/uniprotkb/search?query=organism_id:" + txid + "&fields=accession,id,gene_primary,gene_synonym,organism_name, organism_id,protein_name,xref_proteomes,lit_pubmed_id,annotation_score,xref_geneid&format=tsv"
	subdir := "uniprot/"
	ext := ".upt"
...
// very slow, why ??
func saveAllIdmap(datdir string, txmap util.Set2D) error {
...
func saveAllOnto(datdir string) {
	...
	// https://data.bioontology.org/ontologies/OMIM/submissions/23/download?apikey=8b5b7825-538d-40e0-9e9e-5ab9274a9aeb
...
func main() {
	...
	if *aP || *uP {
		...
		// this should be run from the current 'uniprot' directory
		// TODO implement properly
		// ~/repos/bgw3/scripts/download20up.sh & // TODO provide path for downloadin
		// take ~1 hour
